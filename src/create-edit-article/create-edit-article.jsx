...
import articleIcon from '../assets/newspaper-icon-input.png';
const SectionUpload = () => {
 ...
 const [mainImage, setMainImage] = useState(null); // За главната снимка
 const [mainImageName, setMainImageName] = useState(''); // За името ѝ
 ...
 const handleSubmit = async (e) => {
  ...
  const updatedForm = {
   ...
   image,
   main_image: mainImage, // Главна снимка (ще се прати при всяка секция, но бекендът ще я записва само ако не е празна)
  };
 ...
 return (
  <div className="flex-horizontal-container-raw justify-content-center">
   ...
   <div className={blockAdding ? 'container' : 'container block-adding-article'}>
    ...
    <InfoDiv />
    <div className="text">
     <img src={articleIcon} alt="icon" height="20" />
    ...
    <form onSubmit={handleSubmit} className="flex-vertical-container-raw flex-item align-items-center">
     <input name="article_id" className="margin-10" type="number" style={{ display: 'none' }} placeholder="Article ID" onChange={handleInputChange} value={newArticleId > 0 ? newArticleId : ''} />
     <div className="text">
     ...
     {/* НОВ бутон за главна снимка */}
     <div className="file margin-top-30 file--uploading" style={{ marginBottom: '10px' }}>
      <input id="main-image-file" onChange={handleMainImageChange} type="file" />
      <label htmlFor="main-image-file" className="input-file">
       <img src={upload} alt="upload" />
       <p>Качване на ГЛАВНА снимка (корица)</p>
      </label>
      <p className="upload-image-name">{mainImageName}</p>
     </div>
